...
#' @title returns the list of all the fishstocks found in the database.
#'
#' @description The function is a
#' wrapper around the ICES webservice API
#' \href{http://standardgraphs.ices.dk/standardgraphswebservices.asmx/getListStocks}{getListStocks}
#'
#' @export
#'
#' @return Returns the year of the assessment, the species , the fishstock nam,
#' the ecoregion and the key of the fishstocks/assessment year.
#'
#' @param year The year of the assessment. If the year is 0 then it will return
#' all the assessment years. If there are no matches, an empty result set will
#' be returned.

get_stock_list <- function(year)
  {
  ...
  # ----------------------------------------------------------------------------
  # get stuff-ed
  URL <- "http://standardgraphs.ices.dk/standardgraphswebservices.asmx/getListStocks"
  x <- httr::GET(URL,query=list(year=year))

  # ----------------------------------------------------------------------------
  # XML way
  d = httr::content(x)
  d <- plyr::ldply(XML::xmlToList(d), data.frame, stringsAsFactors=FALSE)
  d$Status <- stringr::str_trim(d$Status)
  d$key <- as.integer(d$key)
  d$AssessmentYear <- as.integer(d$AssessmentYear)
  return(d)

}


#' @title Returns the Summary table of the stock
#'
#' @description The function is a
#' wrapper around the ICES webservice API
#' \href{http://standardgraphs.ices.dk/standardgraphswebservices.asmx/getListStocks}{getListStocks}
...
#' the ecoregion and the key of the fishstocks/assessment year.
#'
#' @param key The key of the assessment. Example would be 1786 for the Cod in
#' Subarea IV (North Sea), Divison VIId (Eastern Channel) and IIIa West (Skagerrak) (cod-347d)
#' in the assessment year of 2013. If there are no matches, an empty result set will be returned.

get_summary_table <- function(key)
{

  if(missing(key)) stop("key must be specified")

  # ----------------------------------------------------------------------------
  # get stuff-ed
  URL <- "http://standardgraphs.ices.dk/standardgraphswebservices.asmx/getSummaryTable"
  x <- httr::GET(URL,query=list(key=key))

  # ----------------------------------------------------------------------------
  # Format
  d <- XML::xmlRoot(httr::content(x))
  d <- XML::xmlToList(d)

  # a bit brute
  d <- plyr::ldply(d[[6]], data.frame, stringsAsFactors=FALSE)
  d <- reshape2::melt(d,c(".id", "Year"))
  d$value <- as.numeric(d$value)
  x <- reshape2::dcast(d,.id + Year ~ variable,value.var = "value")
